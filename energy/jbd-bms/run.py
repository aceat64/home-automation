...
def setup(client, config):
    for sensor in config["sensor"]:
        ...
        sensor[
            "availability_topic"
        ] = f"homeassistant/sensor/{config['pack']['name']}/status"
        ...
        logging.debug(
            f"Topic: homeassistant/{component}/{config['pack']['name']}/{sensor_path_name}/config"
        )
...
if __name__ == "__main__":
    ...
    client.will_set(
        f"homeassistant/sensor/{config['pack']['name']}/status", "offline", 0, True
    )
    ...
    try:
        ...
        while True:
            ...
            if info:
                ...
                client.publish(
                    f"homeassistant/sensor/{config['pack']['name']}/state",
                    json.dumps(info),
                    0,
                    False,
                )
