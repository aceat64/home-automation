...
def getInfo(port):
    ...
    info["dc"]["current_used"] = round(
        calcValue("\x05", unpack("<I", dc_frame[8:11] + bytes([0x00]))[0]), 2
    )
    info["dc"]["current_charge"] = round(
        calcValue("\x05", unpack("<I", dc_frame[11:14] + bytes([0x00]))[0]), 2
    )
    info["dc"]["inverter_freq"] = round(
        10 / calcValue("\x07", unpack("<B", dc_frame[14:15])[0]), 2
    )
...
def setup(client, config):
    for sensor in config["sensor"]:
        ...
        sensor[
            "state_topic"
        ] = f"homeassistant/sensor/{config['inverter']['name']}/state"
        sensor[
            "availability_topic"
        ] = f"homeassistant/sensor/{config['inverter']['name']}/status"
        ...
        logging.debug(
            f"Topic: homeassistant/{component}/{config['inverter']['name']}/{sensor_path_name}/config"
        )
...
if __name__ == "__main__":
    ...
    client.will_set(
        f"homeassistant/sensor/{config['inverter']['name']}/status", "offline", 0, True
    )
